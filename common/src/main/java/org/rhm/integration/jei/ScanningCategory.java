...
public class ScanningCategory implements IRecipeCategory<ShapelessRecipe> {
    ...
    private final MutableText title = Text.translatable(CyberRewaredMod.MOD_ID + ".jei.scan");
    ...
    @Override
    public void draw(@NotNull ShapelessRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull DrawContext guiGraphics, double mouseX, double mouseY) {
    ...
    public void setRecipe(@NotNull IRecipeLayoutBuilder builder, @NotNull ShapelessRecipe recipe, @NotNull IFocusGroup focuses) {
        ...
        builder.addOutputSlot(129, 8).addItemStack(recipe.getResult(null));
    }

    @Override
    @SuppressWarnings("rawtypes")
    public void createRecipeExtras(IRecipeExtrasBuilder builder, ShapelessRecipe recipe, @NotNull IFocusGroup focuses) {
        IScannable scannable = (IScannable) recipe.getIngredients().getFirst().getMatchingStacks()[0].getItem();
        int maxTicks = scannable.getScanTime() == -1 ? ScannerBlockEntity.TICKS_PER_OPERATION : scannable.getScanTime();
        int arrowX = 41;
        ...
        Text timeString = Text.translatable("gui.jei.category.smelting.time.seconds", maxTicks/20);
