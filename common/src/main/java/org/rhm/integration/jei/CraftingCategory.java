...
public class CraftingCategory implements IRecipeCategory<EngineeringCraftRecipe> {
    ...
    private final MutableText title = Text.translatable(CyberRewaredMod.MOD_ID + ".jei.craft");
    ...
    @Override
    public void draw(@NotNull EngineeringCraftRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull DrawContext guiGraphics, double mouseX, double mouseY) {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, EngineeringCraftRecipe recipe, @NotNull IFocusGroup focuses) {
        ...
        builder.addInputSlot(101, 37).addIngredients(recipe.blueprint());
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                if (recipe.ingredients().size() > slotIndex) {
                    rsb.addIngredients(recipe.ingredients().get(slotIndex));
                }
                slotIndex++;
            }
        }
